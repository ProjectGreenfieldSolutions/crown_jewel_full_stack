...
class PaperTestEntryLithoTestView(DjangoWebOLV):
  """
  ...
  def __init__(self):
    ...
    for col, options in self.default_columns.items():
        if options['label'] in app.storage.user.get("preferences")['page_settings']['paper_test_entry']['litho_paper_test_view_columns']:
